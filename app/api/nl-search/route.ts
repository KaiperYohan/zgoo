...
- **q** (string): 회사명/업종/대표자/법인번호/사업자번호/노트(사용자 코멘트) 본문 자유 검색 (부분일치). 특정 회사명·식별자·코멘트 키워드를 찾는 경우에 사용. 사업자번호는 KODATA 원본 형식대로 저장되어 있으므로 입력값을 그대로 유지할 것 (보통 하이픈 포함, 예: "123-45-67890").
...
const APPLY_FILTERS_TOOL: Anthropic.Tool = {
  ...
  input_schema: {
    type: 'object',
    properties: {
      q: { type: 'string', description: '자유 텍스트 검색 (회사명/업종/대표자/법인번호/사업자번호/노트 본문 부분일치)' },
